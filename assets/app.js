...
var header = {
  ...
  dynamicDropdown: function() {
  	var $header = $(".header");
  	if($header.hasClass('header_bar') && $header.data("dropdown-position") == "below_header") {
  	  $('.dropdown').css({"top": 0, "padding-top": $('.header').height()});
  	} else {
  	  $('.dropdown').css({"top": "auto"});
  	}
  },
  ...
  init: function(){
    $('.homepage_slider').each(function (index, value){
      ...
      if ($(this).hasClass('mobile')) {
        controlContainer = $('.number_roots.mobile');
      } else {
        controlContainer = $('.number_roots.desktop');
      }
...
var announcement_bar = {
  ...
  setPadding: function() {
    ...
    var bannerHeight = $('.promo_banner').innerHeight();
    var bannerPadding = (bannerHeight - 25) + 'px';
    var parentSection = $('.promo_banner').parent('.header-section');
...
var banner = {
  ...
  responsiveBanner: function(mobileBanner) {
...
var product = {
  init: function(){
    ...
    $('.product_slider').flexslider({
      ...
      touch: true,
      pauseOnHover: true,
      controlsContainer: $('.number_roots'),
      controlNav: true,
      directionNav: thumbnailPosition == "no-thumbnails" ? true : false,
      customDirectionNav: $('.direction_controls a'),
  ...
  function refreshCart(cart) {
    ...
    if ($cartBtn && $cartBtn.length) {
      ...
      if (cart.item_count == 0) {
        $cart.append('<li class="empty_cart">Your Shopping bag is feeling empty.</li>');
      } else {
        $(".empty_cart").hide();
...
/*             '<p class="mm-counter">' +
            ...
            '</p>' +        */
           '<span class="remove_item mm-listitem__text">'+
            '<a href="/cart/change?line=1&amp;quantity=0" title="Remove"><img src="https://cdn.shopify.com/s/files/1/0158/2484/6912/files/close_ico.svg?12656" class="cart_close"></a>'+
            '</span>'+
            '<a href="' + item.url +'">';
          if (item.image) {
          ...
          if (item.discounts.length) {
            ...
            discountMessage = '<span class="notification-discount"><strong>Discount:</strong> ' + title + ' (-<span class="money">' + discountAmount + '</span>)</span>';
          }
          ...
          if (item.price > item.final_price) {
            ...
            cart_items_html += '<div class="price">' + itemPrice + '</div>' + discountMessage +'</div></a></li>'
          } else {
            var itemPrice = '<span class="money">' + Shopify.formatMoney(item.price, $('body').data('money-format')) + '</span>';
            cart_items_html += '<div class="price">' + itemPrice + '</div></div></a></li>';
          }
        });
        ...
        cart_items_html += '<li class="mm-label">' +
          ...
          cart_items_html += '<span class="mm-action_buttons"><a href="/cart" class="action_button edit_cart left button secondary">Go to cart</a>' +
          '<input type="submit" class="action_button right" value="Checkout" />' + '</span>' + '</li>';
...
$(document).ready(function(){
    jQuery(document).on("click",".menu_toggle_section",function(){
		if(jQuery(this).hasClass("active_menu")){
			console.log("working full");
			jQuery(".menu_type_mobile").hide();
			jQuery(".menu_toggle_section").removeClass("active_menu");
        }
		else{
			console.log("working full not");
			jQuery(".menu_type_mobile").show();
			jQuery(this).addClass("active_menu");
        }
	});
 });
$(document).ready(function(){
	$(document).on("click",".change_slide_sect",function(){
		var current_back_icon = $(this).attr('id');
		var last_active = $(".slides .flex-active-slide").attr('style');
		var last_active_id = $(".slides .flex-active-slide").attr('id');
		var current_active = $(".slides #"+current_back_icon).attr('style');
		console.log(current_back_icon);
		console.log(last_active_id);
		if($(".slides #"+current_back_icon).hasClass("flex-active-slide")){
			console.log("working");
        }
		else{
			$(".slides li").removeClass('flex-active-slide');
			$(".slides #"+current_back_icon).addClass('flex-active-slide');
			$(".slides #"+current_back_icon).attr('style',last_active);
			$(".slides #"+last_active_id).attr('style',current_active);
...
jQuery(document).ready(function($){
	$('.404').addClass('four0four');
	/*jQuery(document).on("click",".icon-menu",function(){
		if(jQuery(".mm-menu_theme-black").hasClass("active_open")){
			jQuery(".mm-menu_theme-black").removeClass("mm-menu_opened");
			jQuery("#mm-2").removeClass("mm-panel_opened");
			jQuery(".mm-menu_theme-black").removeClass("active_open");
			jQuery(".mm-menu_theme-black").addClass("mm-menu_closed");
			jQuery("#mm-2").addClass("mm-panel_closed");
			//jQuery(this).removeClass("active_menu_top");
			jQuery(".slider-bullet").show();
          jQuery(".header_last_report").hide();
			jQuery(".icon-menu").show();
        }
		else{
			jQuery("#nav").removeClass("mm-menu_closed");
			jQuery("#mm-2").removeClass("mm-panel_closed");
			jQuery("#nav").addClass("mm-menu_opened");
			jQuery("#mm-2").addClass("mm-panel_opened");
			jQuery("#nav").addClass("active_open");
			//jQuery(this).addClass("active_menu_top");
			jQuery(".slider-bullet").hide();
          setTimeout(function(){
			jQuery(".header_last_report").removeClass("mm-panel");
            jQuery(".header_last_report").removeClass("mm-hidden");
            jQuery(".header_last_report").removeClass("mm-panel_has-navbar");
			jQuery(".header_last_report").show();
			jQuery(".icon-menu").hide();
          },1000);

        }
    });*/
	/*jQuery(document).on("click",".header_last_report",function(){
		jQuery(".mm-menu_theme-black").removeClass("mm-menu_opened");
        jQuery("#mm-2").removeClass("mm-panel_opened");
        jQuery(".mm-menu_theme-black").removeClass("active_open");
        jQuery(".mm-menu_theme-black").addClass("mm-menu_closed");
        ...
        jQuery(".header_last_report").hide();
        jQuery(".icon-menu").show();
		jQuery(".mediaqueries").removeClass("mm-wrapper_blocking");
    });*/

	jQuery(document).on("click",".toggle_menu_sel",function(){
    jQuery(".header_last_report").show();
    jQuery(".custom_header_menu").addClass("open-menu");
    jQuery(".toggle_menu_sel").hide();
    jQuery("#content_wrapper.mm-page").addClass("backdrop");
  });
	jQuery(document).on("click",".header_last_report",function(){
    jQuery(".header_last_report").hide();
    jQuery(".custom_header_menu").removeClass("open-menu");
    jQuery(".toggle_menu_sel").show();
    jQuery("#content_wrapper.mm-page").removeClass("backdrop");
  });

	jQuery(document).on("click",".hide_button",function(event){
		event.preventDefault();
		jQuery(this).prev('p').hide();
		jQuery(this).text('READ ALL');
		jQuery(this).addClass("action_button");
		jQuery(this).removeClass("hide_button");
    });
	jQuery(document).on("click","#shopify-section-1568795402889 .action_button",function(event){
		event.preventDefault();
		jQuery(this).prev('p').show();
		jQuery(this).text('HIDE');
		jQuery(this).addClass("hide_button");
		jQuery(this).removeClass("action_button");
    });
  	jQuery(".sign_up").val('subscribe');
	jQuery(".flex-direction-nav").insertAfter(".slider-arrows-custom .inner-arrows");
var a=1;
  jQuery(".number_roots li a").each(function(){
      jQuery(this).text("0"+a);
      a++;
  });
	jQuery(".number_roots").show();
});
jQuery(document).ready(function(){
	jQuery(document).on("click",".quatity_increase_decrease .fa-plus",function(){
		//console.log();

		var increase_quant = parseInt(jQuery(this).parent('.quatity_increase_decrease').find('input').val());
      console.log(increase_quant);
		var new_quant_of_product = increase_quant+1;
      console.log(new_quant_of_product);
		jQuery(this).parent('.quatity_increase_decrease').find('input').val(new_quant_of_product);
      $('.update_cart').trigger('click');
    });
	jQuery(document).on("click",".quatity_increase_decrease .fa-minus",function(){
		   var tt = $(this);
		var increase_quant = parseInt(jQuery(this).parent('.quatity_increase_decrease').find('input').val());
      console.log(increase_quant);
		var new_quant_of_product = increase_quant-1;
...
jQuery(document).ready(function(){
	jQuery(document).on("click",".about_less_button",function(event){
		event.preventDefault();
    jQuery(this).parent().find(".full_text").hide();
    jQuery(this).parent().find(".excerpts").show();
		jQuery(this).addClass("about_read_button");
		jQuery(this).removeClass("about_less_button");
		jQuery(this).text('READ ALL');
    });
});
jQuery(document).ready(function(){
	jQuery(document).on("click",".about_read_button",function(event){
		event.preventDefault();
		jQuery(this).parent().find(".full_text").show();
    jQuery(this).parent().find(".excerpts").hide();
		jQuery(this).addClass("about_less_button");
		jQuery(this).removeClass("about_read_button");
		jQuery(this).text("HIDE");
    });
});
jQuery(document).ready(function($){
	jQuery(document).on("click",".page-film img.my_video_open_section",function(e){
		jQuery(this).addClass('my_video_section');
		jQuery(this).prev('div').hide();
		jQuery(this).next('div').show();
    jQuery(this).hide();
    jQuery(this).next('div').find('iframe')[0].src += "&autoplay=1";
    jQuery(this).next('div').find('.close_menu_tag_video').hide();
    jQuery(this).prev().hide();
    jQuery(this).parent().find('.my_play_btn').hide();
    e.preventDefault();
  });
});
jQuery(document).ready(function($){
	jQuery(document).on("click",".page-film .my_play_btn",function(e){
		jQuery(this).addClass('my_video_section');
		jQuery(this).hide();
    jQuery(this).parent().find('.my_video_open_section').hide();
		//$('.my_video_section').append('<style>.my_video_section:before{display:none !important;}</style>');
		jQuery(this).next('img').hide();
		jQuery(this).parent().find('.video-wrapper').show();
    jQuery(this).parent().find('.video-wrapper iframe')[0].src += "&autoplay=1";
    jQuery(this).parent().find('.video-wrapper .close_menu_tag_video').hide();
    jQuery(this).next().hide();
    e.preventDefault();
  });

  jQuery(document).on("click",".page-film .close_menu_tag_video",function(){
		jQuery(this).parent('div').prev().prev('div').show();
		jQuery(this).parent('div').prev('img').show();
		jQuery(this).parent('div').hide();
    jQuery(this).parents('.image_column').find('.my_play_btn').show();
  });
...
function resizeForm(){
  ...
  if(width > 800) {
	jQuery(document).on('click', '.order-history-tabs .icon-arrow-down', function() {
      jQuery('.order-history-tabs li').each(function(){
        jQuery(this).show();
      });
      jQuery(this).addClass('icon-arrow-up').removeClass('icon-arrow-down');
    });

    jQuery(document).on('click', '.order-history-tabs .icon-arrow-up', function() {
      jQuery('.order-history-tabs li').each(function(){
        jQuery(this).hide();
      });
      jQuery('.order-history-tabs li.active').show();
      jQuery(this).addClass('icon-arrow-down').removeClass('icon-arrow-up');
    });
  } // if not mob menu
  else{
	jQuery(document).on('touchstart', '.order-history-tabs .icon-arrow-down', function() {
      jQuery('.order-history-tabs li').each(function(){
        jQuery(this).show();
      });
      jQuery(this).addClass('icon-arrow-up').removeClass('icon-arrow-down');
    ...
    jQuery(document).on('touchstart', '.order-history-tabs .icon-arrow-up', function() {
      jQuery('.order-history-tabs li').each(function(){
        jQuery(this).hide();
      });
      jQuery('.order-history-tabs li.active').show();
...
$(document).ready(function(){
  //if ($(window).width() < 768) {
    ...
    $('.permotion_slider').slick({
      ...
      dots: false,
      arrows:true,
      nextArrow: '<img class="ArrRight" src="//cdn.shopify.com/s/files/1/0158/2484/6912/t/9/assets/arrow_right.svg?v=17549450123291021812" />',
      prevArrow: '<img class="ArrLeft" src="//cdn.shopify.com/s/files/1/0158/2484/6912/t/9/assets/arrow_left.svg?v=16692987437982983857" />',
      infinite: true,
      cssEase: 'linear'
    });

  $('.permotion_sliderDesk').slick({
      slidesToShow: 4,
      slidesToScroll: 1,
      dots: false,
      arrows:true,
  ...
  $('.custom-wrapper .dot_nav a').on('click', function(e){
    e.preventDefault();
    var dataId = $(this).data('target');

    $('.custom-wrapper li').each(function(){
      $(this).find('a').removeClass('active').addClass('normal');
    });
    $(this).addClass('active');
    $('.custom-wrapper .desc .box-desc .item').each(function(){
    ...
    $('.custom-wrapper .box2 img').each(function(){
      if ($(this).data('id') == dataId) {
        $(this).addClass('active').removeClass('normal');
      } else {
        $(this).removeClass('active').addClass('normal');
  ...
  $('.custom-wrapper .box2 img').on('click', function(){
    $('.custom-wrapper .right_bottom_btn a.right').trigger('click');
  });
  $('.custom-wrapper .right_bottom_btn a.right').on('click', function(e){
    e.preventDefault();

    var $content = $(this).parent().parent().prev().find('.item.active');
    if ($content.next().length > 0) {
      $content.removeClass('active').addClass('normal');
      $content.next().addClass('active').removeClass('normal');
    } else {
      $content.removeClass('active').addClass('normal');
      $(this).parent().parent().prev().find('.item:first').addClass('active').removeClass('normal');
    }

    var $image = $(this).parent().prev().find('img.active');
    if ($image.next().length > 0) {
      $image.removeClass('active').addClass('normal');
      $image.next().addClass('active').removeClass('normal');
    } else {
      $image.removeClass('active').addClass('normal');
      $(this).parent().prev().find('img:first').addClass('active').removeClass('normal');
    }

    var active_dot = $('.custom-wrapper .dot_nav a.active');
    if ($(active_dot).parent().next().length > 0) {
      $(active_dot).removeClass('active');
      $(active_dot).parent().next().find('a').addClass('active');
    } else {
      $(active_dot).removeClass('active');
      $('.custom-wrapper .dot_nav li:first a').addClass('active');
    }
  });

  $('.custom-wrapper .right_bottom_btn a.left').on('click', function(e){
    e.preventDefault();

    var $content = $(this).parent().parent().prev().find('.item.active');
    if ($content.prev().length > 0) {
      $content.removeClass('active').addClass('normal');
      $content.prev().addClass('active').removeClass('normal');
    }

    var $image = $(this).parent().prev().find('img.active');
    if ($image.prev().length > 0) {
      $image.removeClass('active').addClass('normal');
      $image.prev().addClass('active').removeClass('normal');
    }

    var active_dot = $('.custom-wrapper .dot_nav a.active');
    if ($(active_dot).parent().prev().length > 0) {
      $(active_dot).removeClass('active');
      $(active_dot).parent().prev().find('a').addClass('active');
    }
  });

  $('.ing_popup_link').on('click', function(e){
    e.preventDefault();
    $('#ingredients').fancybox({
      wrapCSS: 'ing_popup',
      padding: [50, 60, 50, 60],
      scrolling: false,
      maxWidth: 600,
  ...
  $(window).scroll(function(){
    if($(window).scrollTop() + $(window).height() == $(document).height()) {
     $('.coll_filter').css('bottom', '58px');
   } else {
     $('.coll_filter').css('bottom', 0);
   }
  });
});
...
$(document).ready(function() {
  ...
  $('.product .slider_controls').css('height', $(window).height() + 'px');
  $('body #content_wrapper').css('min-height', $(window).height() + 'px');

  $('.ingredient_links a').on('click', function(){
    var id = $(this).data('target');
    $('.ingredient_links a').each(function(){
      $(this).removeClass("active");
    });
    $('.ingredient_content').each(function(){
      $(this).hide();
    });
    $(this).addClass('active');
    $('#' + id).show();
  ...
  $('.custom_header_menu .custom_menu_inner i.fa').on('click', function(){
    ...
    $(window).scroll( function(){
      var OuterheightHalf= ($(this).outerHeight())/2;
      if ($('.image-banner-fadeIn-main').length > 0) {
        var bottom_of_object = $('.image-banner-fadeIn-main').offset().top+ OuterheightHalf;
        var bottom_of_window = $(window).scrollTop() + $(window).height();
        //console.log("bottom_of_object: "+bottom_of_object+" bottom_of_window: "+bottom_of_window);
        /* If the object is completely visible in the window, fade it it */
        if( bottom_of_window > bottom_of_object ){
          $('.image-banner-fadeIn-main').animate({'opacity':'1'},700);
        }
      }
    });
    $(window).resize(function(){

      // ingredient carousel content height
      var height = $('.procustom .row .two-thirds.second').height() - 1;
      $('.procustom .desc').height(height + 'px');

      if ($('.product_gallery').length) {
        var top_margin = $('.product_gallery').height() / 2 + 170;
        $('.product .number_roots .flex-control-nav').css('top', top_margin + 'px');
      }
      if ($('.index .slideshow-section .desktop-only .homepage_slider').length) {
        var top_margin = $('.index .slideshow-section .desktop-only .homepage_slider').height() / 2;
        $('.index .number_roots .flex-control-nav').css('bottom', 'auto');
        $('.index .number_roots .flex-control-nav').css('transform', 'translateY(-50%)');
        $('.index .number_roots').css('top', top_margin + 'px');
      }

      if ($('.allabout .main-image').length && ($(window).width() >= 768)) {
        var img_height = $('.allabout .main-image').height();
        $('.allabout .icons.alpha').css('padding-top', img_height / 2 - 60 + 'px');
      }

      if ($('.slideshow-section .desktop-only').length && ($(window).width() >= 768)) {
        var max_height = 0;
        $('.slideshow-section .desktop-only .slides li').each(function(){
      ...
      if (($(window).width() >= 768) && ($('body.collection').length > 0)) {
        var min_height = $(window).height() - 67;
        $('.global-wrapper').css('min-height', min_height + 'px');
        if ($('.global-wrapper').height() <= $(window).height()) {
          $('.coll_filter').css('bottom', '58px');
        }
      }

      if ($(window).width() < 768) {
        if ($('.slideshow-section .mobile-only .video_slide').length > 0) {
          var video_height = $('.slideshow-section .mobile-only .video_slide iframe').height();
          $('.slideshow-section .mobile-only .slides li').each(function(){
            $(this).find('.hsContainer').css('min-height', video_height + 'px');
          });
        }
      }

      if ($(window).height() < 700) {
        $('.custom_header_menu .custom_menu_inner ul').css('max-height', '390px');
        $('.custom_header_menu .custom_menu_inner ul').css('overflow-y', 'scroll');
      }
    }).resize();
